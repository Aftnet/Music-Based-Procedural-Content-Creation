...
AudioSectionStr& AudioSections::Section(unsigned int Index)
{
...
AudioSectionStr* AudioSections::GetSectionsArrPtr(void)
{
...
void AudioSections::Generate(AudioFeatures& Input, float SectionLenInMsecs)
{
	...
	for(i=0;i<m_uiNumSections;i++)
	{
		...
		if(m_pSectionsArr[i].BeatLocations.size() > 1)
		{
			...
			j = m_pSectionsArr[i].BeatLocations.size()-1;
			j = (((j*j)-j)/2)+j;
			IntervalsVec.clear();
			...
			for(j=0;j<IntervalDurationsOccurrences.size();j++)
			{
				...
				for(k=0;k<IntervalDurationsOccurrences[j].MemberIntervals.size();k++)
				{
					...
					if(NewStartBeat == true)
					{
						...
						while(Column.size() > 0)
						{
							while(Column[Column.size()-1].size() > 0)
							{
								Row.clear();
							...
							{
								IntervalDurationsOccurrences[j].MaxChainNumEl = Column.size()-2;
							}
			...
			if(IntervalDurationsOccurrences.size()==0)
			{
			...
			else
			{
				...
				AvgIntervalLen = AvgIntervalLen/double(j);
