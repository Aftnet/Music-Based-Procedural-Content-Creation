...
WindowFeatures& AudioFeatures::Window(unsigned int Index)
{
...
void AudioFeatures::CalculateRhythmInterpolation(void)
{
	...
	{
		//WeightsArr[SideSamplesToInclude+i] = 1.0;
		WeightsArr[SideSamplesToInclude+i] = BellMaxHeight*exp(0.0-((pow(double(i),2.0))/(2.0*(pow(BellWidthParam,2.0)))));
	}
	...
	for(i=0;i<m_uiNumWindows;i++)
	{
		...
		for(j=0;j<NumWeights;j++)
		{
			...
			if(TempIndex>=0 && TempIndex<int(m_uiNumWindows))
			{
				...
				//m_pWindowFeaturesPtr[i].RhythmInterpolation += WeightsArr[j]*fabs(m_pWindowFeaturesPtr[TempIndex].FreqCentroidDelta);
				m_pWindowFeaturesPtr[i].RhythmInterpolation += WeightsArr[j]*fabs(m_pWindowFeaturesPtr[TempIndex].FreqSpreadDelta);
				//m_pWindowFeaturesPtr[i].RhythmInterpolation += WeightsArr[j]*m_pWindowFeaturesPtr[TempIndex].GroupDelay;
			}
		}
	}
	delete[] WeightsArr;
